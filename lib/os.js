...
OSTask.prototype.applyParam = function(param) {
  if (param) {
    if (this.conf.param) {
      this.conf.param = combineObject(this.conf.param);
    } else {
  ...
  if (this.conf.param) {
    ...
    if (this.conf.from) {
      for (var i = 0; i < this.conf.from.length; ++i) {
        this.conf.from[i] = strformat(this.conf.from[i], param);
      }
    }

    if (this.conf.to) {
      for (var i = 0; i < this.conf.to.length; ++i) {
        this.conf.to[i] = strformat(this.conf.to[i], param);
      }
    }
  }
}
...
OSContext.prototype.addTask = function (task) {
  task.applyParam(this.conf.param);
  var name = task.name;
