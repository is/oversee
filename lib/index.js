...
OSContext.prototype.tidy = function() {
  ...
  for (var tid in this.uncheckedTasks) {
    ...
    if (!conf.slot in this.slots) {
      ...
      log.debug("TIDY", {reason: "invalid-slot-id", task:task.name, id: conf.slot});
      continue;
    }
...
OSContext.prototype.tick = function() {
  ...
  if (this.waitTasks.length + this.readyTasks.length + this.runTasks.length > 0) {
    this.callId = setTimeout(function() { self.tick();}, 500);
  }
};


OSContext.prototype.fire = function(task) {
  this.readyTasks.splice(this.readyTasks.indexOf(task), 1);
  this.slots[task.conf.slot].rank += task.capacity;

  this.runTasks.push(task);
  task.state = OSTask.STATE_RUN;
  task.child = cp.spawn(task.conf.cmd, tasks.conf.args, task.conf.opts);

  var self = this;
  task.child.on('exit', function(code) {
    self.onTaskExit(task, code);
  })
};
...
OSContext.prototype.onTaskExit = function(task, code) {
  ...
  if (code == 0) {
    ...
    for (int i = 0; i < task.blocks; ++i) {}
      var name = task.blocks[i];
      var task2 = this.tasks[name];

      task2.depends.splice(task2.depends.indexOf(name), 1);
    ...
    if (task.blocks.length != 0) {
      ...
      while (concelList.length) {
        ...
        if (btask.blocks.length) {
          for (int i = 0; i < btask.blocks.length; ++i) {
...
OSContext.prototype.moveTask(task, from, to) {
