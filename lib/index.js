...
OSContext.prototype.taskSpawnOpts = function(task) {
  ...
  if (this.conf.task_log == OSContext.LOG_FILE) {
    ...
    task.logOut = fs.openSync(task.logPath, this.conf.logfile_openmode);
    task.logErr = fs.openSync(task.logPath, this.conf.logfile_openmode);

    opts.stdio = ['ignore', task.logOut, 'ignore'];
  }
