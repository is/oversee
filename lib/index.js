...
OSContext.prototype.setup = function(cf) {
  ...
  if ('OSEE_ROOT' in process.env) {
    defaultConfig.home_dir = process.env['OSEE_ROOT'];
  }
  ...
  this.log = jog('OSC:' + this.conf.name);
...
OSContext.prototype.run = function() {
  ...
  if (this.waitTasks.length
   ...
   + this.runTasks.length > 0) {
    ...
    jog('MAIN').info('END', {name: this.conf.name});
  }
};
...
exports.startup = function() {
  ...
  var argv = optimist
    ...
    .alias('p', 'param')
    .alias('c', 'config')
    .default('base', process.env.OSEE_ROOT ? env.OSEE_ROOT: '.home')
    .default('name', 'one')
    .argv;

  jog('MAIN').info('START', {name: argv.name});
